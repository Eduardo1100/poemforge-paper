from __future__ import annotations

import argparse
import json
from pathlib import Path

import numpy as np
import pandas as pd
from sklearn.linear_model import LogisticRegression
from sklearn.metrics import accuracy_score
from sklearn.model_selection import StratifiedKFold
from sklearn.pipeline import make_pipeline
from sklearn.preprocessing import StandardScaler


ROOT = Path(__file__).resolve().parents[2]
OUT_DIR = ROOT / "results" / "litbench"


def bootstrap_accuracy(correct: np.ndarray, n_boot: int, seed: int) -> dict[str, float]:
    rng = np.random.default_rng(seed)
    correct = np.asarray(correct, dtype=float)
    n = len(correct)

    boots = np.empty(n_boot, dtype=float)
    for b in range(n_boot):
        idx = rng.integers(0, n, size=n)
        boots[b] = float(np.mean(correct[idx]))

    return {
        "accuracy": float(np.mean(correct)),
        "ci95_low": float(np.quantile(boots, 0.025)),
        "ci95_high": float(np.quantile(boots, 0.975)),
        "n": int(n),
        "n_boot": int(n_boot),
    }


def load_inputs(tag: str, nll_tag: str) -> pd.DataFrame:
    surface_path = OUT_DIR / f"litbench_surface_features_{tag}.csv"
    nll_path = OUT_DIR / f"litbench_nll_scores_{nll_tag}.csv"

    if not surface_path.exists():
        raise FileNotFoundError(surface_path)
    if not nll_path.exists():
        raise FileNotFoundError(nll_path)

    surf = pd.read_csv(surface_path)
    nll = pd.read_csv(nll_path)

    cols = [
        "row_id",
        "chosen_avg_nll",
        "rejected_avg_nll",
        "chosen_total_nll",
        "rejected_total_nll",
        "chosen_n_loss_tokens",
        "rejected_n_loss_tokens",
    ]
    df = surf.merge(nll[cols], on="row_id", how="inner", validate="one_to_one")
    return df


def make_pairwise_examples(df: pd.DataFrame, feature_cols: list[str], seed: int) -> tuple[np.ndarray, np.ndarray]:
    rng = np.random.default_rng(seed)
    rows = []
    labels = []

    for _, row in df.iterrows():
        chosen = []
        rejected = []
        for base in feature_cols:
            chosen.append(row[f"chosen_{base}"])
            rejected.append(row[f"rejected_{base}"])

        chosen = np.asarray(chosen, dtype=float)
        rejected = np.asarray(rejected, dtype=float)

        # Randomize orientation to avoid label-position leakage.
        if rng.random() < 0.5:
            rows.append(chosen - rejected)
            labels.append(1)
        else:
            rows.append(rejected - chosen)
            labels.append(0)

    return np.vstack(rows), np.asarray(labels, dtype=int)


def crossval_accuracy(X: np.ndarray, y: np.ndarray, n_splits: int, seed: int) -> np.ndarray:
    skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=seed)
    preds = np.zeros_like(y)

    for train_idx, test_idx in skf.split(X, y):
        clf = make_pipeline(
            StandardScaler(),
            LogisticRegression(max_iter=1000, C=1.0),
        )
        clf.fit(X[train_idx], y[train_idx])
        preds[test_idx] = clf.predict(X[test_idx])

    return preds == y


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--surface-tag", default="test_ids_complete")
    ap.add_argument("--nll-tag", default="test_ids_complete_distilgpt2")
    ap.add_argument("--tag", default="test_ids_complete_distilgpt2")
    ap.add_argument("--n-splits", type=int, default=5)
    ap.add_argument("--n-boot", type=int, default=5000)
    ap.add_argument("--seed", type=int, default=123)
    args = ap.parse_args()

    df = load_inputs(args.surface_tag, args.nll_tag)

    feature_sets = {
        "surface_format": [
            "chars",
            "words",
            "type_token_ratio",
            "avg_word_len",
            "punct_count",
            "newline_count",
            "paragraph_count",
        ],
        "nll_avg_only": [
            "avg_nll",
        ],
        "surface_plus_avg_nll": [
            "chars",
            "words",
            "type_token_ratio",
            "avg_word_len",
            "punct_count",
            "newline_count",
            "paragraph_count",
            "avg_nll",
        ],
        "surface_plus_avg_and_total_nll": [
            "chars",
            "words",
            "type_token_ratio",
            "avg_word_len",
            "punct_count",
            "newline_count",
            "paragraph_count",
            "avg_nll",
            "total_nll",
            "n_loss_tokens",
        ],
    }

    rows = []
    pred_cols = {"row_id": df["row_id"].to_numpy()}

    for name, features in feature_sets.items():
        X, y = make_pairwise_examples(df, features, seed=args.seed)
        valid = np.isfinite(X).all(axis=1)
        X = X[valid]
        y = y[valid]

        correct = crossval_accuracy(X, y, n_splits=args.n_splits, seed=args.seed)
        stat = bootstrap_accuracy(correct, n_boot=args.n_boot, seed=args.seed)
        rows.append(
            {
                "tag": args.tag,
                "model": name,
                "features": json.dumps(features),
                **stat,
            }
        )

    summary = pd.DataFrame(rows).sort_values("accuracy", ascending=False)

    summary_path = OUT_DIR / f"litbench_combined_models_{args.tag}.csv"
    manifest_path = OUT_DIR / f"litbench_combined_models_{args.tag}_manifest.json"

    summary.to_csv(summary_path, index=False)
    manifest_path.write_text(
        json.dumps(
            {
                "analysis": "cross-validated logistic pairwise models",
                "surface_tag": args.surface_tag,
                "nll_tag": args.nll_tag,
                "tag": args.tag,
                "n_splits": args.n_splits,
                "n_boot": args.n_boot,
                "seed": args.seed,
                "outputs": {
                    "summary": str(summary_path),
                },
            },
            indent=2,
        ),
        encoding="utf-8",
    )

    print(summary.to_string(index=False))
    print(f"Wrote {summary_path}")
    print(f"Wrote {manifest_path}")


if __name__ == "__main__":
    main()
